...
class SubmenuService:
    ...
    async def create(self, menu_id: int, data: SubmenuRequest, background_tasks: BackgroundTasks) -> submenu:
        result = await self.db_repo.create(menu_id, data)
        background_tasks.add_task(self.cache.delete, ce.menu_list, ce.menu, ce.submenu_list, menu_id=menu_id)
        background_tasks.add_task(self.cache.set, ce.submenu, result, menu_id=menu_id, submenu_id=result.id)
        return result

    async def update(self, menu_id: int, submenu_id: int, data: SubmenuRequest, background_tasks: BackgroundTasks) -> submenu:
        result = await self.db_repo.update(submenu_id, data)
        background_tasks.add_task(self.cache.delete, ce.submenu_list, ce.submenu,
                                  menu_id=menu_id, submenu_id=submenu_id)
        background_tasks.add_task(self.cache.set, ce.submenu, result, menu_id=menu_id, submenu_id=result.id)
        return result

    async def delete(self, menu_id: int, submenu_id: int, background_tasks: BackgroundTasks) -> None:
        result = await self.db_repo.delete(submenu_id)
        background_tasks.add_task(self.cache.delete, ce.menu_list, ce.menu, ce.submenu_list, ce.submenu_cascade,
                                  menu_id=menu_id, submenu_id=submenu_id)
        return result
