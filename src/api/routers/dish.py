...
async def create(menu_id: int, submenu_id: int, data: DishRequest, service: DishService = Depends()):
...
async def update(menu_id: int, submenu_id: int, dish_id: int, data: DishRequest, service: DishService = Depends()):
    return await service.update(menu_id, submenu_id, dish_id, data)


@router_dish.delete('/{dish_id}', summary='Удаляет блюдо по id')
async def delete(menu_id: int, submenu_id: int, dish_id: int, service: DishService = Depends()):
